import { config, DEFAULT_LIMITS } from '../config.js';
...
export class GitHubClient {
  ...
  async api(path, options = {}) {
    const url = `${config.githubApiBase}${path}`;
    let response;
    try {
  ...
  async getRawFile(owner, repo, ref, path, maxBytes = config.maxFileBytes) {
    const url = `${config.rawBase}/${owner}/${repo}/${encodeURIComponent(ref).replace(/%2F/g, '/')}/${path.split('/').map(encodeURIComponent).join('/')}`;
    let response;
    try {
  ...
  async loadPullRequestSnapshot({ owner, repo, pullNumber, repoMeta, scope = 'standard', maxFiles }) {
    ...
    const selected = prFiles
      ...
      .slice(0, maxFiles || ({ quick: 60, standard: 120 }[scope] || 120));
